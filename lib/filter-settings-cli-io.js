...
export default class FilterSettingsCliIO {
  async input () {
    ...
    filter.siteOrDomain = await this.#typeIn('site or domain', "'Search one site (like wikipedia.org ) or limit your results to a domain like .edu, .org or .gov'")
    filter.lastUpdate = await this.#selectFromList('lastUpdate', 'last update')
    filter.termsAppearing = await this.#selectFromList('termsAppearing', 'terms appearing')
    filter.safeSearch = await this.#selectFromList('safeSearch', 'safe search (whether to filter sexually explicit content)')
    filter.fileType = await this.#selectFromList('fileType', 'file type')
    filter.usageRights = await this.#selectFromList('usageRights', 'usage rights')
