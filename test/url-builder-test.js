...
describe("the input formatting function of UrlBuilder", function () {
  ...
  it("escapes characters for url", async () => {
    const result = urlBuilder.formatInputForURL(
      "they are escaped ;,/?:@&=+$# and they are not -_.!~*'()"
    );
    const expected =
      "they+are+escaped+%3B%2C%2F%3F%3A%40%26%3D%2B%24%23+and+they+are+not+-_.!~*'()";
    assert.strictEqual(result, expected);
  });
...
describe("the URL generation function of UrlBuilder", function () {
  it("generates Google Search URL with given query fields and filter (using the filter of which all operators are set to initial defaults.)", async () => {
    ...
    const urlBuilder = new UrlBuilder();
    const expected =
      "https://www.google.com/search?as_q=aaa&as_epq=bbb&as_oq=ccc&as_eq=ddd&as_nlo=10&as_nhi=100&lr=&cr=&as_qdr=all&as_sitesearch=&as_occt=any&safe=images&as_filetype=&tbs=";
  ...
  it("generates Google Search URL with given query fields and filter (using the filter of which all operators are set to non-default)", async () => {
    ...
    const urlBuilder = new UrlBuilder();
    const expected =
      "https://www.google.com/search?as_q=what+is+bullet+train%3F&as_epq=&as_oq=&as_eq=movie&as_nlo=&as_nhi=&lr=lang_en&cr=countryUK&as_qdr=y&as_sitesearch=wikipedia.org&as_occt=body&safe=active&as_filetype=pdf&tbs=sur%3Af";
    const result = urlBuilder.generateGoogleSearchUrl(query, filter);
  ...
  it("returns an url even if the query fields are all empty", async () => {
    ...
    const urlBuilder = new UrlBuilder();
    const expected =
      "https://www.google.com/search?as_q=&as_epq=&as_oq=&as_eq=&as_nlo=&as_nhi=&lr=&cr=&as_qdr=all&as_sitesearch=&as_occt=any&safe=images&as_filetype=&tbs=";
    const result = urlBuilder.generateGoogleSearchUrl(query, filter);
